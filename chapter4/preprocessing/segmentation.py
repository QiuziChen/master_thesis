...
import pandas as pd
from preprocessing.calculation import calTimeInterval
...
def segment(
    ...
    # generate tripID
    traj.loc[:, tripIDColName] = None
    traj.loc[segID, tripIDColName] = list(range(len(segID)))
    traj[tripIDColName].fillna(method='ffill', inplace=True)
